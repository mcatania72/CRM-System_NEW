...
# =============================================================================
# LOCAL BUILD + SSH DEPLOY SCRIPT - TAR FIXED VERSION
# =============================================================================
...
ARCHIVE_DIR="/tmp/crm-archive-$(date +%Y%m%d_%H%M%S)"

# Debug paths
echo -e "${BLUE}[DEBUG]${NC} SCRIPT_DIR: $SCRIPT_DIR"
echo -e "${BLUE}[DEBUG]${NC} PROJECT_ROOT: $PROJECT_ROOT"
echo -e "${BLUE}[DEBUG]${NC} BUILD_DIR: $BUILD_DIR"
echo -e "${BLUE}[DEBUG]${NC} ARCHIVE_DIR: $ARCHIVE_DIR"
echo ""
...
EOF
    ...
    # NUOVO APPROCCIO TAR - Directory separata per archivio
    log_info "Creando archivio deployment..."
    
    # Crea directory separata per archivio
    mkdir -p "$ARCHIVE_DIR"
    
    # Crea archivio da directory separata
    tar -czf "$ARCHIVE_DIR/crm-deployment.tar.gz" -C "$BUILD_DIR" .
    
    # Sposta archivio nella build dir per compatibilità
    mv "$ARCHIVE_DIR/crm-deployment.tar.gz" "$BUILD_DIR/"
    
    # Cleanup directory archivio
    rm -rf "$ARCHIVE_DIR"
    
    log_success "Package deployment creato: $BUILD_DIR/crm-deployment.tar.gz"
    log_info "Dimensione: $(du -h $BUILD_DIR/crm-deployment.tar.gz | cut -f1)"
}
